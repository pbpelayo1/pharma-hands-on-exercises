...
```{r}
adsl_counts <- function(Big_N_cnt, small_n_cnt){
  adsl_mrg_cnt <- small_n_cnt %>%
    left_join(Big_N_cnt, by = c("TRT01A", "TRT01AN")) %>%
    mutate(perc = round((n/N)*100, digits=1)) %>%
    ungroup() %>%
...
test_that("calculate_small_n_cnt() output has expected dim, columns, and types", {
  ...
  expect_equal(dim(calculate_small_n_cnt(test_data)), c(length(unique(calculate_small_n_cnt(test_data)$TRT01A))*length(unique(calculate_small_n_cnt(test_data)$SEX)), 4))
})
...
test_that("adsl_counts() output has expected dim, columns, types, and values", {
  adsl_test_data <-  test_data %>% 
    adsl_counts(., Big_N_cnt, small_n_cnt)
  
  expect_named(adsl_test_data, c("TRT01A","SEX", "perc"))
  expect_equal(dim(adsl_test_data), c(length(unique(adsl_test_data$TRT01A))*length(unique(adsl_test_data$SEX)), 3))
  expect_type(adsl_test_data$perc, "double")

  #test if calculated percent is correct
  adsl_mrg_cnt <- small_n_cnt %>%
    left_join(Big_N_cnt, by = c("TRT01A", "TRT01AN"))
  adsl_mrg_cnt$perc = round(adsl_mrg_cnt$n / adsl_mrg_cnt$N * 100, digit=1)
    
  
  expect_identical(adsl_test_data$perc, adsl_mrg_cnt$perc)
})
