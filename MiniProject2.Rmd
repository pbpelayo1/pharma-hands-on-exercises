...
Big_N_cnt_new <- adsl_eff %>%
  group_by(TRT01AN, TRT01A, AGEGR1N, AGEGR1) %>%
  count(name = "N")
Big_N_cnt_new

small_n_cnt_new <- adsl_eff %>%
...
Table_Challenge1 <- adsl_mrg_cnt_new %>%
  pivot_wider(names_from = TRT01A, values_from = npct)
...
```{r}
rawdata <- 
  import (file = "https://github.com/phuse-org/phuse-scripts/raw/master/data/adam/cdisc/adae.xpt")
rawdata
