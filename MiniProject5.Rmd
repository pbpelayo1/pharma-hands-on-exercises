...
```{r}
vs_plot <- function(plot_template, paramn){
  param_data <- ADVS %>% 
    filter(PARAMN== {{paramn}}) %>% 
    filter(VISITNUM > 3)
  
    param_plot <- {{plot_template}} %+%
      param_data
    
    xLab <- quo_name(param_plot$mapping$x)
    yLab <- quo_name(param_plot$mapping$y)
    groupLab <- quo_name(param_plot$mapping$group)
    
    study <- uniqueVal(param_plot$data$STUDYID)
    measure <- uniqueVal(param_plot$data$PARAM)
...
DiaBP_plot <- vs_plot(plot4, 2)
Temp_plot <- vs_plot(plot4, 6)
...
ADVS2 <- vs_s %>% filter(!is.na(vs_s$AVAL))
...
xLab <- quo_name(Pulse2_plot$mapping$x)
yLab <- quo_name(Pulse2_plot$mapping$y)
groupLab <- quo_name(Pulse2_plot$mapping$group)
 
study <- uniqueVal(Pulse2_plot$data$STUDYID)
measure <- uniqueVal(Pulse2_plot$data$PARAM)
